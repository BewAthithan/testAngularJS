                ...
                controller: ['$scope', '$http', '$q', 'ngAuthSettings', '$state', 'pageLoading', '$window', 'commonService', '$timeout', '$translate', 'localStorageService', '$interval', 'warehouseFactory',
                    function ($scope, $http, $q, ngAuthSettings, $state, pageLoading, $window, commonService, $timeout, $translate, localStorageService, $interval, warehouseFactory) {
                        ...
                        $scope.onClose = function (param) {
                            var param = {};
                            // param.warehouseIndex = null;
                            // param.warehouseId = null;
                            // param.warehouseName = null;
                            // $scope.invokes.selected(param);
                            $scope.onShow = false;
                        };
                        $scope.$watch('actionPS', function(newCodes, oldCodes){
                            $scope.actionPS = newCodes;
                            $scope.find();
                        });
                        ...
                        $scope.filter = function () {
                            ...
                            if($scope.actionPS == '1') {
                                $scope.warehouse.warehouseId = document.getElementById('warehouseIdValue').value;
                                $scope.warehouse.warehouseName = "";
                            } else if($scope.actionPS == '2') {
                                $scope.warehouse.warehouseId = "";
                                $scope.warehouse.warehouseName = document.getElementById('warehouseNameValue').value;
                            }
