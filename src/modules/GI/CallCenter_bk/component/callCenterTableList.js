...
app.component('callCenterTableList', {
    ...
    controller: function ($scope, $filter, $q, $compile, $http, /*ngAuthSettings,*/ $state, /*authService,*/ pageLoading, $window, commonService, localStorageService, $timeout, $translate, dpMessageBox, callCenterFactory, planGoodsIssueItemFactory) {
        ...
        $scope.Close = function () {
            ...
            $scope.filterModel.listCallCenterModel[0].ReasonCodeIndex = $scope.filterModel.ReasonCodeIndex;
            $scope.filterModel.listCallCenterModel[0].ReasonCodeId = $scope.filterModel.ReasonCodeId;
            $scope.filterModel.listCallCenterModel[0].ReasonCodeName = $scope.filterModel.ReasonCodeName;
            $scope.filterModel.listCallCenterModel[0].IsAccept = $scope.filterModel.IsAccept;
            $scope.filterModel.listCallCenterModel[0].IsDissmiss = $scope.filterModel.IsDissmiss;
        ...
        $scope.reasonCode = function (model) {
            $scope.CloseCall = 0;

            $scope.filterModel = model;
            if (model != null) {
                $scope.popupReasonCode.onClick(model);
            }
        };

        $scope.save = function () {
            $scope.filterModel.CreateBy = $scope.model.CreateBy;
