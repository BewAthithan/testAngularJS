'use strict'
app.component('putToStagingV2', {
    controllerAs: '$vm',
    templateUrl: function ($element, $attrs, $window, commonService) {
        return "modules/GI/PutToStagingV2/PutToStagingV2.html";
    },
    bindings: {
        isLoading: '=?',
        searchResultModel: '=?',
    ...
    controller: function ($scope, $filter, $q, $compile, $http, $state, pageLoading, $window, commonService, localStorageService, $timeout, dpMessageBox, taskListFactory, cartNumberFactory, locationFactory, cartAssignFactory, $interval) {
        ...
        $scope.ConfirmCarton = function () {
            ...
            validate(models).then(function (result) {
                if (result) {
                    ...
                    viewModelCartNumber.cartConfirmDropV2(models).then(function success(res) {
        ...
        var init = function () {
            ...
            if (item != undefined) {
                ...
                pageLoading.show();
                viewModelCartNumber.cartPutToStagingV2(objitem).then(function success(res) {
                    pageLoading.hide();
