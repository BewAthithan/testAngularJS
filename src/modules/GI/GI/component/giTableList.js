...
app.component('giTableList', {
    ...
    controller: function ($scope, $filter, $q, $compile, $http, /*ngAuthSettings,*/ $state, /*authService,*/ pageLoading, $window, commonService, localStorageService, $timeout, $translate, dpMessageBox, goodIssueFactory, goodsIssueItemFactory,runWaveFactory) {
        var $vm = this;
        ...
        $scope.undo = function (param) {
            ...
            dpMessageBox.confirm({
                ...
                viewModel.Undo(param.goodsIssueIndex,$scope.userName).then(function success(res) {
        ...
        $scope.mend = function(param){
            MessageBox.confirm({
                ok: 'Yes',
                cancel: 'No',
                title: 'Confirm.',
                message: 'Do You Want to Repair Data ?'
            }).then(function success() {
                mend(param);
            });
        }
        ...
        function mend(param) {
            let item = {};
            item.goodsIssueIndex = param.goodsIssueIndex
            item.Create_By = $scope.userName
            viewModelRunwave.RunWave(item).then(
                function success(results) {
                    var contentArr = results.data.split(',');

                    if (contentArr[0] == "Wave ไม่สมบูรณ์") {
                        MessageBox.confirm({
                            ok: 'Yes',
                            cancel: 'No',
                            title: 'Confirm.',
                            message: "Order " + contentArr[1] + " นี้ ไม่สมบูรณ์ ต้องการ ซ่อมดาต้าหรือไม่"
                        }).then(function success() {
                            mend(param)
                        });
                    }
        ...
        $scope.mendPickmanual = function(param){
            MessageBox.confirm({
                ok: 'Yes',
                cancel: 'No',
                title: 'Confirm.',
        ...
        function mendPickmanual(param) {
            ...
            item.planGoodsIssueIndex.push(param.refDocumentIndex);
            item.goodsIssueIndex = param.goodsIssueIndex
            item.Create_By = $scope.userName
