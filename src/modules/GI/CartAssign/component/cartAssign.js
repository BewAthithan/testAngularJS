(function () {
    ...
    app.component('cartAssign', {
        controllerAs: '$vm',
        templateUrl: "modules/GI/CartAssign/component/cartAssign.html",
        bindings: {
            onShow: '=?',
            searchResultModel: '=?',
            filterModel: '=?',
        ...
        controller: function ($scope, $filter, $http, $state, pageLoading, $window, localStorageService, $translate, $q, dpMessageBox, cartAssignFactory, equipmentFactory, $interval) {
            ...
            $vm.CartList = function () {
                ...
                //     if ($scope.isLoading) {
                //         $vm.isFilter = false;                                       
                //         $scope.isLoading(models.equipmentItemName).then(function () {
                //             $vm.isFilter = true;
                //             $scope.isLoadingPicking = true;
                //         }).catch(function (error) {
                //             defer.reject({ 'Message': error });
            ...
            $scope.ScanPickTicket = function () {
                ...
                $scope.SendPickTicket(models).then(function success(res) {
                    ...
                    // setTimeout(() => {
                    //     var focusElem = jQuery('input[ng-model="filterModel.equipmentItemName"]');
