'use strict'
app.component('autoPosReconfirmTableList', {
    controllerAs: '$vm',
    templateUrl: function ($element, $attrs, /*ngAuthSettings,*/ $window, commonService) {
        return "modules/GI/AutoPOSReconfirm/component/posReconfirmTableList.html";
    },
    bindings: {
        isLoading: '=?',
        searchResultModel: '=?',
    ...
    controller: function ($scope, $filter, $q, $compile, $http, /*ngAuthSettings,*/ $state, /*authService,*/ pageLoading, $window, commonService, localStorageService, $timeout, $translate, dpMessageBox, planGoodsIssueFactory, autoPosFactory, posFactory) {
        var $vm = this;
        var XFindItem = $filter('findItemList');
        var Progressbar = pageLoading;
        $scope.items = $scope.items || [];
        var viewModel = autoPosFactory;
            ...
            dpMessageBox.confirm({
                ...
                viewModel.checkUser(param.posIndex).then(function (res) {
                    ...
                    if (res.data) {
                        ...
                        viewModel.ConfirmPackedReconfirm(dataConfirm).then(function success(res) {
        ...
        $scope.ConfirmShipment = function (param) {
            dpMessageBox.confirm({
                ...
                cancel: 'No',
                title: 'InformaTion',
                message: 'Do you want to ReConfirmShipment ?'
            }).then(function success() {
                ...
                if(idx.length > 1)
                {
                    dpMessageBox.alert({
                        ok: 'OK',
                        title: 'Error',
                        message: 'กรุณายืนยัน Reconfirm shipment ครั้งละ 1 รายการ'
                    });
                    return;
                }
        ...
        $scope.serchPage = function (param) {
            autoPosFactory.filter(param).then(function success(res) {
